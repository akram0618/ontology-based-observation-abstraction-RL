...
class Simulator:
    ...
    def from_json(cls, conf_file):
        ...
        percent_to_replace = 0.05  # 2 percent
        ...
        #weather.temperatures['Hour_Category'] = weather.temperatures['Hour'].apply(cls.categorize_hour)
        #grouped_df = weather.temperatures.groupby('Hour_Category')['Temperature'].mean().reset_index()
        #average_temp_dict = dict(zip(grouped_df['Hour_Category'],grouped_df['Temperature']))
        #weather.temperatures.loc[indices_to_replace, column_to_replace] = weather.temperatures['Hour_Category'].map(average_temp_dict)
        weather.temperatures['Day_Type'] = ''

        # Step 3: Use pandas datetime functions to extract the day of the week (0: Monday, 6: Sunday)
        weather.temperatures['Date'] = pd.to_datetime(weather.temperatures[['Year', 'Month', 'Day']])
        weather.temperatures['Day_of_Week'] = weather.temperatures['Date'].dt.dayofweek

        # Step 4: Classify days as "free day" or "working day" based on the day of the week
        weather.temperatures.loc[weather.temperatures['Day_of_Week'].isin([5, 6]), 'Day_Type'] = 'Free Day'  # Weekends (Saturday and Sunday)
        weather.temperatures.loc[weather.temperatures['Day_of_Week'].isin([0, 1, 2, 3, 4]), 'Day_Type'] = 'Working Day'  # Weekdays (Monday to Friday)
        grouped_df = weather.temperatures.groupby('Day_Type')['Temperature'].mean().reset_index()
        average_daytype_dict = dict(zip(grouped_df['Day_Type'], grouped_df['Temperature']))
        weather.temperatures.loc[indices_to_replace, column_to_replace] = weather.temperatures['Day_Type'].map(
            average_daytype_dict)
